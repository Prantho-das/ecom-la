<section class="bg-white relative overflow-hidden">
  <div class="flex flex-wrap">
    <!-- Left Image -->
    <div class="w-full lg:w-3/5">
      <img
        src="/assets/images/why-choose-1.jpg"
        alt="Engineer with wind turbine models"
        class="w-full h-full object-cover"
      />
    </div>

    <!-- Right Image -->
    <div class="w-full lg:w-2/5">
      <img
        src="/assets/images/why-choose-2.jpg"
        alt="Engineer with wind turbine models"
        class="w-full h-full object-cover"
      />
    </div>
  </div>

  <!-- Floating Content Box -->
  <div class="absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 lg:translate-x-0 bg-[#27ad4c] text-white md:p-12 p-8 shadow-2xl lg:-ml-48 md:max-w-[550px] w-4/5">
    <p class="font-semibold mb-2 capitalize">why choose us</p>

    <h2 class="md:text-4xl text-3xl font-bold md:mb-6 mb-4">
      We are Building a Sustainable Future
    </h2>

    <!-- List -->
    <ul class="space-y-6">
      <li class="flex">
        <div class="text-2xl mt-1">✅</div>
        <div class="ml-4">
          <h3 class="font-bold text-lg">Robust Experience</h3>
          <p class="text-sm opacity-90">
            Vast experience in power evacuation, land pro curement, liaisoning and working with state.
          </p>
        </div>
      </li>
      <li class="flex">
        <div class="text-2xl mt-1">✅</div>
        <div class="ml-4">
          <h3 class="font-bold text-lg">Best Energy Tariffs</h3>
          <p class="text-sm opacity-90">
            A perfect blend of global experience and expertise to further our focus on technology.
          </p>
        </div>
      </li>
    </ul>
  </div>
</section>
